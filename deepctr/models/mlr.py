...
def get_region_score(features,feature_columns, region_number, l2_reg, init_std, seed,prefix='region_',seq_mask_zero=True):
    ...
    region_logit =concat_fun([get_linear_logit(features, feature_columns,
                                    l2_reg, init_std, seed+i, prefix=prefix+str(i+1)) for i in range(region_number)])
...
def get_learner_score(features,feature_columns, region_number, l2_reg, init_std, seed,prefix='learner_',seq_mask_zero=True,task='binary'):
    region_score = [PredictionLayer(task=task,use_bias=False)(get_linear_logit(features, feature_columns,
                                                l2_reg, init_std, seed + i, prefix=prefix + str(i + 1))) for i in
                               range(region_number)]

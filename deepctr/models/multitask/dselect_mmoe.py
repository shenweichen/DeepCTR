...
def dselectMMOE(dnn_feature_columns, num_experts=3, expert_dnn_hidden_units=(256, 128), tower_dnn_hidden_units=(64,),
         gate_dnn_hidden_units=(), l2_reg_embedding=0.00001, l2_reg_dnn=0, seed=1024, dnn_dropout=0,
         dnn_activation='relu',
         dnn_use_bn=False, task_types=('binary', 'binary'), task_names=('ctr', 'ctcvr'),n_select=2):
    ...
    """
    assert n_select<=num_experts,'n_select must <= num_experts'
    ...
    expert_concat = expert_outs # list of tensors consist of expert outputs

    mmoe_outs = []
    for i in range(num_tasks):  # one mmoe layer: nums_tasks = num_gates
        gate_mul_expert = DSelectKGate(n_select)(expert_concat)
